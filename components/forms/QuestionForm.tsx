...
const QuestionForm = ({ mongoUserId }: { mongoUserId: string }) => {
  ...
  const handleInputTag = (e: React.KeyboardEvent<HTMLInputElement>, field: any) => {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex w-full flex-col gap-3">
                ...
                <FormDescription className="mt-2.5 font-spaceGrotesk text-[14px] font-normal leading-[19.6px] text-light-500">
                  Introduce the problem and expand what you put in the title. At least 20 characters.
                </FormDescription>
                <FormMessage className="text-red-500" />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex w-full flex-col">
                ...
                <FormControl className="mt-3.5">
                  <>
                    <Input onKeyDown={(e) => handleInputTag(e, field)}
                    ...
                    {field.value.length > 0 && <div className='flex-start mt-2.5 gap-2.5'>
                      {field.value.map((tag: any) => {
                        return <Badge onClick={() => handleTagRemove(tag, field)} key={tag} className='background-light800_dark300 subtle-medium text-light400_light500 flex items-center justify-center gap-2 rounded-md border-none px-4 py-2 uppercase'>
                          {tag}
                          <Image src="/assets/icons/close.svg" alt="close" width={12} height={12} className='cursor-pointer object-contain invert-0 dark:invert' />
                        </Badge>
                      })}
                    </div>}
                  </>
